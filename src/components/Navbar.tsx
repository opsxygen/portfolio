  ...
  return (
    <nav className="sticky top-0 z-20 py-3 bg-white w-full border-b border-gray-200 flex gap-2 items-center justify-end">
      ...
      >
        ...
        <div
          className={cn(
            "w-full z-50 pt-10 overflow-hidden transition-all duration-300 ease-in-out overflow-y-auto",
            menuOpen ? "max-h-max py-4" : "max-h-0 py-0"
          )}
        >
          <NavLinks onClose={() => setMenuOpen(false)} isSidebarOpen={true} />
        ...
        {menuOpen && (
          <div className={"grid grid-cols-2 gap-4"}>
            <Button
              variant="outline"
              size="sm"
              className="flex text-xs px-4 py-[1rem] items-center gap-2 rounded-full font-normal w-full"
            >
            ...
            <Link
              target="_blank"
              href={siteSettings?.bookAppointmentLink || "/"}
            >
              <Button
                variant="default"
                size="sm"
                className="flex text-xs px-4 py-[1rem] items-center justify-center gap-2 rounded-full font-normal w-full group"
              >
                Book an appointment
                <Image
                  src="/arrow.svg"
